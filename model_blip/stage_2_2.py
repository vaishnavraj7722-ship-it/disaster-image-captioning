...
def setup_logging(output_dir: str):
    ...
    log_file = os.path.join(output_dir, f"stage1_blip2_{timestamp}.log")
    logging.basicConfig(
        level=logging.INFO,
...
def save_checkpoint(model, processor, optimizer, scheduler, epoch, path):
    ...
    # With LoRA, this saves only the small adapter weights, not the full base model
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Stage 1: Flickr8k BLIP-2 Fine-Tuning (8-bit + LoRA)")
    ...
    logger.info("Loading BLIP-2 base model in 8-bit...")
    ...
    logger.info(f"Total params: {total:,} | Trainable: {trainable:,} ({100*trainable/total:.4f}%)")
    if torch.cuda.is_available():
    ...
    logger.info("Stage 1 (BLIP-2, 8-bit + LoRA) complete!")
