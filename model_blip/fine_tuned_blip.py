#!/usr/bin/env python3
"""
Stage 1: Flickr8k Fine-Tuning (v2 - regularized)
============================
Fine-tunes BLIP on Flickr8k with ALL 5 captions per image.
Vision encoder frozen, lower LR, early stopping added to prevent overfitting.

Usage:
    python model_blip/fine_tuned_blip.py \
        ...
        --output_dir ~/blip_disaster/checkpoints/stage1_v2 \
...
def setup_logging(output_dir: str):
    ...
    log_file = os.path.join(output_dir, f"stage1_{timestamp}.log")
    logging.basicConfig(
        level=logging.INFO,
...
class Flickr8kDataset(Dataset):
    """
    Loads Flickr8k captions.txt.
    CRITICAL: Keeps ALL 5 captions per image (no deduplication).
    """
...
def main():
    ...
    parser.add_argument("--data_path", required=True, help="Path to captions.txt")
    ...
    logger.info("Vision encoder frozen. Only text decoder will be fine-tuned.")

    total = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f"Total params: {total:,} | Trainable: {trainable:,} ({100*trainable/total:.1f}%)")
    ...
    logger.info("Next: Test this checkpoint on a few images, then run stage2_disaster.py")
